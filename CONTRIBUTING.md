...
LaunchDarkly employees can reach out with questions or comments in [our Slack channel, #proj-ds-npm](https://launchdarkly.slack.com/channels/C039XACMX4Z)
...
- **Perform a cursory search in the [#proj-ds-npm channel](https://launchdarkly.slack.com/channels/C039XACMX4Z) to see if the problem has already been reported. If it has **and the issue is still open**, add a comment to the existing issue instead of opening a new one.**
- **Let us know in detail in the [#proj-ds-npm channel](https://launchdarkly.slack.com/channels/C039XACMX4Z) (with screenshots or video if possible) what the issue is and how to reproduce it.**

### Suggesting Enhancements

We're always open to enhancement suggestions! When assessing an enhancement, we must consider how it will affect other teams, how it will function with other UI/UX patterns in Launchpad, and more. Feel free to start a discussion in the [#proj-ds-npm channel](https://launchdarkly.slack.com/channels/C039XACMX4Z).
...
- Duplicate an existing packages directory, like `alert`, and change its name to your component name in the singular (`notification`, not `notifications`).
...
- Replace `lodash` as a dependency in favor of modularized npm dependencies. For example, replace `import {filter} from 'lodash';` with `import filter from 'lodash.filter'`.
...
- Run `npx changeset` before making a pull request. For a completely new package, you'll tell Changeset you're making a "minor" version update.
- If you had to change any dependency packages, those might be either "minor" or "patch" updates, depending on what you did.
...
You're ready to go! Just post a PR and tag someone from the `UX Next` squad to take a look. Happy launching.
