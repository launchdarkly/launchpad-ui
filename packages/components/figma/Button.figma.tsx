...
figma.connect(Button, 'https://www.figma.com/design/98HKKXL2dTle29ikJ3tzk7/LaunchPad?node-id=1-27006', {
	props: {
		hasIconLeft: figma.boolean('Has icon left', {
			true: figma.instance('Left icon').render<IconProps>(({ name }) => <Icon name={name} size="small" />),
			false: undefined,
		}),
		hasIconRight: figma.boolean('Has icon right', {
			true: figma.instance('Right icon').render<IconProps>(({ name }) => <Icon name={name} size="small" />),
			false: undefined,
		}),
