...
const AUTO_GENERATED_BANNER = 'This file is auto-generated by scripts/figma-icons.ts. Do not edit manually.';
...
async function fetchWithTimeout<T>(p: Promise<T>, ms: number): Promise<T> {
	const t = new Promise<never>((_, rej) => setTimeout(() => rej(new Error(`Request timed out after ${ms}ms`)), ms));
...
async function fetchSvgUrls(components: ComponentRecord[]): Promise<Record<string, string>> {
	...
	await mapPool(batches, URL_RESOLVE_CONCURRENCY, async (batch, batchIdx) => {
		...
		log('fetch', `🔗 Requesting image URLs for batch #${batchIdx + 1} (${batch.length} component(s))`);
