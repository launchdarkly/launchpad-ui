/**
 * Refresh icons from Figma, optimize them with SVGO, emit a single SVG sprite,
...
async function http<T>(url: string, init?: RequestInit): Promise<T> {
	const doOnce = async () => {
		const res = await withTimeout(fetch(url, init), FETCH_TIMEOUT_MS);
		if (!res.ok) {
...
function optimizeSvg(svg: string, debugName: string): string {
	// Intentionally strict: remove potential scripting/embedding & normalize output.
	const { data } = optimize(svg, {
		...
		plugins: [
			...
			{ name: 'removeTitle' },
			{ name: 'removeDesc' },
			// keep stroke/fill *attributes* only if author explicitly needs them as currentColor etc.
...
async function main(): Promise<void> {
	...
	// 5) Build *deterministic*, fully-sorted sprite
