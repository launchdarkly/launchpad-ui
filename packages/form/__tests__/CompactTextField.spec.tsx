...
describe('CompactTextField', () => {
	...
	it('calls onFocus when focused', async () => {
		...
		const spy = vi.fn();
		render(<CompactTextField label="Email" aria-label="Email" onChange={() => undefined} onFocus={spy} />);
	...
	it('calls onBlur when blurred', async () => {
		const user = userEvent.setup();
		const spy = vi.fn();
		render(<CompactTextField label="Email" aria-label="Email" onChange={() => undefined} onBlur={spy} />);
