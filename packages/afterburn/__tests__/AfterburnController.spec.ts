...
import { AfterburnController } from '../src/AfterburnController';
...
describe('AfterburnController', () => {
	let controller: AfterburnController;
	const defaultConfig = {
	...
	afterEach(() => {
		...
		document
			.querySelectorAll('.afterburn-tooltip, .afterburn-settings, .afterburn-settings-trigger')
	...
	it('toggles afterburn on keyboard shortcut', () => {
		controller = new AfterburnController(defaultConfig);
	...
	it('shows settings trigger when active', () => {
		...
		const trigger = document.querySelector('.afterburn-settings-trigger');
	...
	it('does not show tooltips when inactive', () => {
		...
		// Don't activate afterburn (should be inactive by default)
		expect(document.body.classList.contains('afterburn-active')).toBe(false);
	...
	it('shows Text and Heading components when enabled in settings', () => {
		...
		document.body.classList.add('afterburn-show-text');
