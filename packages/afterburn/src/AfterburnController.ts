...
import { generateDocsUrl } from './utils/attribution';

/**
 * Minimal vanilla JS tooltip system for LaunchPad Afterburn
 * Provides hover tooltips with component information without React overhead
 */
export class AfterburnTooltip {
	...
	private handleMouseOver(e: MouseEvent) {
		// Only show tooltips when Afterburn is active
		if (!document.body.classList.contains('afterburn-active')) {
			return;
		}
	...
	private shouldShowComponent(componentName: string): boolean {
		...
		if (componentName === 'Text' || componentName === 'Heading') {
			// Only show if the afterburn-show-text class is present
			return document.body.classList.contains('afterburn-show-text');
		}
	...
	private handleMouseOut(e: MouseEvent) {
		...
		if (
			...
			if (
				relatedTarget.closest('.afterburn-tooltip') ||
	...
	private handleDocumentClick(e: MouseEvent) {
		...
		if (target && typeof target.closest === 'function') {
			if (!target.closest('[data-launchpad]') && !target.closest('.afterburn-tooltip')) {
				this.hideTooltip();
			}
		} else {
	...
	private createTooltip(
		...
		tooltip.className = 'afterburn-tooltip';
		...
		tooltip.innerHTML = `
			<div class="afterburn-tooltip-header">
				<span class="afterburn-tooltip-title">${componentName}</span>
				<span class="afterburn-tooltip-package">${packageName}</span>
			</div>
			<div class="afterburn-tooltip-description">${description}</div>
			<div class="afterburn-tooltip-links">
				<a href="${docsUrl}" target="_blank" rel="noopener noreferrer" class="afterburn-tooltip-link">
					📖 Documentation
				</a>
			</div>
		`;
...
/**
 * Settings panel for LaunchPad Afterburn
 * Provides UI controls for customizing highlighting behavior
 */
class AfterburnSettings {
	...
	private createTrigger() {
		...
		this.trigger.title = 'Afterburn Settings - Click for options, drag to move';
	...
	private updateTriggerPosition() {
		...
		this.trigger.className = `afterburn-settings-trigger afterburn-settings-trigger--${this.currentPosition}`;
	...
	show() {
		...
		const existingTriggers = document.querySelectorAll('.afterburn-settings-trigger');
	...
	hide() {
		...
		const allTriggers = document.querySelectorAll('.afterburn-settings-trigger');
	...
	private createPanel(): HTMLElement {
		...
		panel.className = 'afterburn-settings';

		// Position panel relative to trigger position
		this.updatePanelPosition(panel);

		panel.innerHTML = `
			<div class="afterburn-settings-header">Afterburn Settings</div>
			<div class="afterburn-settings-option">
				<span class="afterburn-settings-label">Show Text & Heading</span>
				<div class="afterburn-toggle ${this.settings.showText ? 'active' : ''}" data-setting="showText">
					<div class="afterburn-toggle-handle"></div>
				</div>
			</div>
			<div class="afterburn-settings-links">
				<a href="https://github.com/launchdarkly/launchpad-ui" target="_blank" rel="noopener noreferrer" class="afterburn-settings-link">
					🔗 GitHub Repository
				</a>
				<a href="https://launchpad.launchdarkly.com" target="_blank" rel="noopener noreferrer" class="afterburn-settings-link">
					📚 Storybook
				</a>
			</div>
		`;
	...
	private applySetting(setting: keyof typeof this.settings) {
		switch (setting) {
			case 'showText':
				document.body.classList.toggle('afterburn-show-text', this.settings.showText);
				break;
		}
	}
...
/**
 * Main controller for LaunchPad Afterburn functionality
 * Handles activation toggle and coordinates CSS highlighting with JS tooltips
 */
export class AfterburnController {
	private tooltip: AfterburnTooltip;
	private settings: AfterburnSettings;
	private keyHandler: (e: KeyboardEvent) => void;
	...
	constructor(
		...
		this.tooltip = new AfterburnTooltip(config.metadata, config.docsBaseUrl);
		this.settings = new AfterburnSettings();
		this.keyHandler = this.handleKeyDown.bind(this);
	...
	enable() {
		...
		// Add click handler to toggle Afterburn when clicked (useful for Storybook)
	...
	private toggle() {
		const isActive = document.body.classList.contains('afterburn-active');
		this.setActive(!isActive);
	}

	private setActive(active: boolean) {
		if (active) {
			document.body.classList.add('afterburn-active');
			this.settings.show();
		} else {
			document.body.classList.remove('afterburn-active');
			document.body.classList.remove('afterburn-show-text'); // Reset text visibility
			this.settings.hide();
		}
	}
}
